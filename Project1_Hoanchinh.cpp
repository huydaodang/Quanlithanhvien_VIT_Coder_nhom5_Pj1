...
int n=0;
char info[30];
Member **InitlistMember(int size)  //Ham tao danh sach cac thanh vien
{
	Member **list;
...
int clean_stdin() // Ham nhan het cac dau xuong dong khi su dung scanf
{
    while (getchar() != '\n') {
    }
    return 1;
...
bool Adjust(char name[30]); //Kiem tra ten 
char *convert( char name[30]); //Revert name Dao Dang Huy -> Huy Dao Dang
void sort_name(Member **list); //Ham sap xep noi bot
char *Format_name(char name[30]); //Ham dinh dang ten 
...
void Writefile(FILE *fout,Member **list);
void Menu(FILE *fout,Member **list);
void Readfile(FILE *fin,Member **list);
int main()
{
	FILE *fout;
	FILE *fin;
...
bool Adjust(char name[30])   // Ham kiem tra ten it nhat phai co 2 thanh phan first name va last name
{
...
{ 
  // VD: Nguyen Van Thuan --> Thuan Nguyen Van,  Tran Thi Thanh Thuy --> Thuy Tran Thi Thanh 
   int k= strlen(name)-1; // k là d? dài c?a tên - 1
   int i=0;
   char ten[30], tam[10];          
  	while (name[k] !=' ')
  	  {
...
void sort_name(Member **list)  //O day ta dung con tro nen doi cho hai vung nho rat kho
{ 							   //Nen o day ta chi doi cho du lieu hai vung nho
  	int ID;
	char Name[30];
	int Act_Point;
	int Gr_Id;	
...
void Add_Member(Member **list)
{
  ...
  char Name[30] ,ch;
        do {
        		list[n]=(Member*)malloc(sizeof(Member));
        		fflush(stdin);
            	system("cls");
...
void Find_Member(Member **list,int size,char info[])
{					//Tim thong tin thanh vien chi can nhap 1 thong tin bat ky
	int i;
	char info1[30];
	strcpy(info1,info);
	Format_name(info1);
...
void Writefile(FILE *fout,Member **list)
{
	for(int i=0 ; i<n ; i++)
	{
		fputs(list[i]->Name,fout);
		fprintf(fout,"\n%d %d %d\n",list[i]->ID,list[i]->Gr_Id,list[i]->Act_Point);
	}
	
	if(n==0)
		printf("Chua co thong tin nao de luu\n");
}
void Readfile(FILE *fin,Member **list)
{
	while(!feof(fin))
	{
		int ID=0 ;  int Act_Point=0 ; int Gr_Id=0 ;	
		char Name[256]; char getId[256]; 
		if(fgets(Name,256,fin) == '\0')
		{
			//printf("Error: Loi ten\n");
			return;
		}
		if(fgets(getId,256,fin) == NULL)
		{
			printf("Error: Loi doc ID\n");
			return;
		}
		if(sscanf(getId,"%d%d%d",&ID,&Gr_Id,&Act_Point)<0)
		{
			printf("Error: Loi chuyen so\n");
			return ;
		}
		list[n]=(Member*)malloc(sizeof(Member));
		...
		convert(Name);
		strcpy(member->Convert_Name,Name);		
		member->Gr_Id=Gr_Id;
		member->Act_Point=Act_Point;
